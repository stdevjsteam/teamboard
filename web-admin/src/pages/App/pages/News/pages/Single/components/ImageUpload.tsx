...
class ImageUpload extends Component<Props, State> {
  ...
  onUpload = async () => {
    ...
    await this.props.dispatch(
      news.uploadPhoto({ file, purpose: "news_photo" })
    );
  ...
  render() {
    ...
    if (_news) {
      fileList.push({
        uid: Date.now(),
        url: API_ROOT + "/" + _news.photo,
        name: "",
        type: "",
        size: 0
      });
    }
    const photo = _news ? _news.photo : "";
    return (
      <Fragment>
        <Upload
          ...
          onRemove={() => {
            this.props.dispatch(
              news.uploadPhoto({ purpose: "user_photo", file: "" })
            );
          }}
