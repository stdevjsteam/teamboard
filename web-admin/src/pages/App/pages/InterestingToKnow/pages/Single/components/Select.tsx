...
type State = {
  interestingToKnowGroups: groups.Member[];
...
class Single extends Component<Props, State> {
  ...
  handleChange = async selectedOption => {
    await this.setState({
      interestingToKnowGroups: selectedOption.map(value => ({
        id: value.value
      }))
    });
    type Obj = { id: number };
    if (this.props.defaultGroups) {
      const selectedGroups = (this.state.interestingToKnowGroups as any).map(
        data => {
          const obj: Obj = {
            id: data.id
          };
      ...
      for (let a in initialObject) {
        if (
          this.state.interestingToKnowGroups.length > 0 &&
